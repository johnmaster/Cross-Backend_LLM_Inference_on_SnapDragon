...
本目录是 `../04_matmul_qhpi_hvx` 的独立优化版本，用于验证“一次计算多行，
...
```text
qnn_custom_ops/04_matmul_qhpi_hvx
```

优化版本：

```text
qnn_custom_ops/05_matmul_qhpi_hvx_multi_row
```
...
PKG=qnn_custom_ops/05_matmul_qhpi_hvx_multi_row/htp/MatMulQhpiHvxMultiRowOpPackage
```
...
/home/lingbok/Qualcomm/qairt/2.47.0.260601/bin/x86_64-linux-clang/qnn-model-lib-generator \
  -c qnn_custom_ops/05_matmul_qhpi_hvx_multi_row/model/custom_matmul_qhpi_hvx_multi_row_model.cpp \
  -t aarch64-android \
  -l custom_matmul_qhpi_hvx_multi_row_model \
  -o qnn_custom_ops/05_matmul_qhpi_hvx_multi_row/model_libs
```

产物：

```text
qnn_custom_ops/05_matmul_qhpi_hvx_multi_row/model_libs/aarch64-android/libcustom_matmul_qhpi_hvx_multi_row_model.so
```
...
adb push \
  qnn_custom_ops/05_matmul_qhpi_hvx_multi_row/input/. \
  /data/local/tmp/qnn/custom_matmul_qhpi_hvx_multi_row/input/
```
...
```bash
rm -rf qnn_custom_ops/05_matmul_qhpi_hvx_multi_row/device_output

adb pull \
  /data/local/tmp/qnn/custom_matmul_qhpi_hvx_multi_row/output \
  qnn_custom_ops/05_matmul_qhpi_hvx_multi_row/device_output
```
...
output = np.fromfile(
    "qnn_custom_ops/05_matmul_qhpi_hvx_multi_row/device_output/Result_0/output.raw",
    dtype=np.float32,
)
expected = np.fromfile(
    "qnn_custom_ops/05_matmul_qhpi_hvx_multi_row/test_data/expected_float.raw",
    dtype=np.float32,
)
...
```text
qnn_custom_ops/18_tools/qnn_sample_app_profile
```
...
```bash
cd qnn_custom_ops/18_tools/qnn_sample_app_profile
