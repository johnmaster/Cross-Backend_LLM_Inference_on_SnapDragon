...
#define MATMUL_QHPI_HVX_PACK_LHS_INTRINSICS 1
#endif

static inline int16_t matmulqhpihvxpacklhsFloatToQ13(float value) {
...
static uint32_t matmulqhpihvxpacklhs_float_16_Execute(
  ...
  uint64_t index = 0;
#if MATMUL_QHPI_HVX_PACK_LHS_INTRINSICS
  ...
  for (; index < elements; ++index) {
    output[index] =
        matmulqhpihvxpacklhsFloatToQ13(static_cast<float>(input[index]));
  }
  return QHPI_Success;
}

static float matmulqhpihvxpacklhs_float_16_CostFunc(
...
static QHPI_Tensor_Signature_v1
    matmulqhpihvxpacklhs_float_16_InputSignatures[] = {{
...
static QHPI_Tensor_Signature_v1
    matmulqhpihvxpacklhs_float_16_OutputSignatures[] = {{
        .element_type = QHPI_Float16,
        .layout = QHPI_Layout_Flat4,
        .storage = QHPI_Storage_Direct,
        .mem_placement = QHPI_MemLoc_DDR_OR_TCM,
...
static QHPI_Kernel_v1 matmulqhpihvxpacklhs_float_16_Kernel = {
    .function_name = "matmulqhpihvxpacklhs_float_16_Execute",
    .function = matmulqhpihvxpacklhs_float_16_Execute,
    ...
    .min_inputs = 1,
    .input_signature = matmulqhpihvxpacklhs_float_16_InputSignatures,
    .min_outputs = 1,
    .output_signature = matmulqhpihvxpacklhs_float_16_OutputSignatures,
    .cost_function = matmulqhpihvxpacklhs_float_16_CostFunc,
    .sync_block_size = 0,
...
static QHPI_Kernel_v1 matmulqhpihvxpacklhsKernels[] = {
    matmulqhpihvxpacklhs_float_16_Kernel,
};

QHPI_OpInfo_v1 matmulqhpihvxpacklhsOpInfo = {
    .name = THIS_PKG_NAME_STR "::MatMulQhpiHvxPackLhs",
    .num_kernels = 1,
    .kernels = matmulqhpihvxpacklhsKernels,
