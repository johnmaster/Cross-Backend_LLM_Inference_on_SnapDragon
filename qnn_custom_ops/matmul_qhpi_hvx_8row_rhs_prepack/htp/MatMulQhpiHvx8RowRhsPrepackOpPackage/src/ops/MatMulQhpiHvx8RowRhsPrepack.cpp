//==============================================================================
// Auto Generated Code for MatMulQhpiHvx8RowRhsPrepack - QHPI Implementation
...
#define MATMUL_QHPI_HVX_8ROW_RHS_PREPACK_INTRINSICS 1
#endif

// Forward declarations for MatMulQhpiHvx8RowRhsPrepack kernel matmulqhpihvx8rowrhsprepack_float_16_
static uint32_t matmulqhpihvx8rowrhsprepack_float_16_Execute(QHPI_RuntimeHandle *handle,
                                      uint32_t num_outputs, QHPI_Tensor **outputs,
                                      uint32_t num_inputs, const QHPI_Tensor *const *inputs);
static float matmulqhpihvx8rowrhsprepack_float_16_CostFunc(const uint32_t num_inputs, const QHPI_Tensor *const *inputs);

// Common forward declarations for MatMulQhpiHvx8RowRhsPrepack
static const QHPI_Op* matmulqhpihvx8rowrhsprepackEarlyRewrite(const QHPI_Op *op);
static QHPI_Shape matmulqhpihvx8rowrhsprepackShapeRequired(const QHPI_Op *op);
static QHPI_Shape matmulqhpihvx8rowrhsprepackShapeLegal(const QHPI_Op *op, const QHPI_Shape* shape);
static const QHPI_Op* matmulqhpihvx8rowrhsprepackBuildTile(const QHPI_Op *op, const QHPI_Shape* start, const QHPI_Shape* extent);
static const QHPI_Op* matmulqhpihvx8rowrhsprepackLateRewrite(const QHPI_Op *op);

static inline float matmulqhpihvx8rowrhsprepackScalarDot(const __fp16 *lhs,
                                           const int16_t *rhs,
                                           uint64_t lhs_base,
                                           uint64_t rhs_base,
                                           uint32_t k,
                                           ...
                                           uint32_t col) {
  float sum = 0.0f;
  constexpr float inv_q13 = 1.0f / 8192.0f;
...
static inline int16_t matmulqhpihvx8rowrhsprepackFloatToQ13(float value) {
...
#if MATMUL_QHPI_HVX_8ROW_RHS_PREPACK_INTRINSICS
static inline void matmulqhpihvx8rowrhsprepackCompute1x64Hvx(const __fp16 *lhs,
                                                       const int16_t *rhs,
                                                       __fp16 *output,
                                                       uint64_t lhs_base,
                                                       uint64_t rhs_base,
                                                       ...
                                                       uint32_t n,
                                                       uint32_t col) {
#if defined(MATMUL_QHPI_HVX_8ROW_RHS_PREPACK_DEBUG_STORE_RHS)
  (void)lhs;
  (void)lhs_base;
  (void)k;
  ...
  for (uint32_t reduction = 0; reduction < k; ++reduction) {
    const int16_t lhs_q13 =
        matmulqhpihvx8rowrhsprepackFloatToQ13(static_cast<float>(lhs[lhs_base + reduction]));
...
static inline void matmulqhpihvx8rowrhsprepackCompute8x64Hvx(
    const __fp16 *lhs,
    const int16_t *rhs,
    __fp16 *output,
    uint64_t lhs_base,
  ...
  for (uint32_t reduction = 0; reduction < k; ++reduction) {
    ...
    const HVX_Vector lhs0 = Q6_Vh_vsplat_R(
        matmulqhpihvx8rowrhsprepackFloatToQ13(
...
/*
 * QHPI Registration using hexnn_ffi.h API for MatMulQhpiHvx8RowRhsPrepack
 * Multiple kernels for different data type combinations
 */


// Input tensor signatures for MatMulQhpiHvx8RowRhsPrepack kernel matmulqhpihvx8rowrhsprepack_float_16_
// Includes both regular inputs and parameters as inputs
static QHPI_Tensor_Signature_v1 matmulqhpihvx8rowrhsprepack_float_16_InputSignatures[] = {
...
static QHPI_Tensor_Signature_v1 matmulqhpihvx8rowrhsprepack_float_16_OutputSignatures[] = {
...
// Kernel definition for MatMulQhpiHvx8RowRhsPrepack kernel matmulqhpihvx8rowrhsprepack_float_16_
static QHPI_Kernel_v1 matmulqhpihvx8rowrhsprepack_float_16_Kernel = {
    .function_name = "matmulqhpihvx8rowrhsprepack_float_16_Execute",
    .function = matmulqhpihvx8rowrhsprepack_float_16_Execute,
    ...
    .min_inputs = 2,
    .input_signature = matmulqhpihvx8rowrhsprepack_float_16_InputSignatures,
    .min_outputs = 1,
    .output_signature = matmulqhpihvx8rowrhsprepack_float_16_OutputSignatures,
    .cost_function = matmulqhpihvx8rowrhsprepack_float_16_CostFunc,
    .sync_block_size = 0,
...
// Array of all kernels for MatMulQhpiHvx8RowRhsPrepack
static QHPI_Kernel_v1 matmulqhpihvx8rowrhsprepackKernels[] = {

    matmulqhpihvx8rowrhsprepack_float_16_Kernel
};

// Operator info for MatMulQhpiHvx8RowRhsPrepack - exported for package registration
QHPI_OpInfo_v1 matmulqhpihvx8rowrhsprepackOpInfo = {
    .name = THIS_PKG_NAME_STR "::" "MatMulQhpiHvx8RowRhsPrepack",
    .num_kernels = 1,
    .kernels = matmulqhpihvx8rowrhsprepackKernels,
    .early_rewrite = matmulqhpihvx8rowrhsprepackEarlyRewrite,
    .shape_required = matmulqhpihvx8rowrhsprepackShapeRequired,
    .shape_legalized = matmulqhpihvx8rowrhsprepackShapeLegal,
    .build_tile = matmulqhpihvx8rowrhsprepackBuildTile,
    .late_rewrite = matmulqhpihvx8rowrhsprepackLateRewrite
};


/* QHPI execute function implementation for MatMulQhpiHvx8RowRhsPrepack kernel matmulqhpihvx8rowrhsprepack_float_16_ */
static uint32_t matmulqhpihvx8rowrhsprepack_float_16_Execute(
  ...
  // The RHS is prepacked as Q13, so each reduction step directly loads it.
  ...
  for (uint32_t b = 0; b < batch; ++b) {
    for (uint32_t h = 0; h < heads; ++h) {
      ...
      uint32_t row = 0;
#if MATMUL_QHPI_HVX_8ROW_RHS_PREPACK_INTRINSICS && \
    !defined(MATMUL_QHPI_HVX_8ROW_RHS_PREPACK_FORCE_SCALAR)
      for (; row + 8 <= m; row += 8) {
        ...
        uint32_t col = 0;
        for (; col + 64 <= n; col += 64) {
          matmulqhpihvx8rowrhsprepackCompute8x64Hvx(
        ...
        for (; col < n; ++col) {
          for (uint32_t tile_row = 0; tile_row < 8; ++tile_row) {
            ...
            output[tile_output_base + col] =
                static_cast<__fp16>(matmulqhpihvx8rowrhsprepackScalarDot(
    ...
    !defined(MATMUL_QHPI_HVX_8ROW_RHS_PREPACK_FORCE_SCALAR)
        for (; col + 64 <= n; col += 64) {
          matmulqhpihvx8rowrhsprepackCompute1x64Hvx(
              lhs, rhs, output, lhs_base, rhs_base, output_base, k, n, col);
        }
#endif
        for (; col < n; ++col) {
...
{
  /*
   * Cost estimation function for MatMulQhpiHvx8RowRhsPrepack kernel matmulqhpihvx8rowrhsprepack_float_16_
...
/*
 * Common stub implementations for MatMulQhpiHvx8RowRhsPrepack QHPI_OpInfo functions
...
{
  /*
   * Early rewrite function for MatMulQhpiHvx8RowRhsPrepack
...
{
  /*
   * Shape required function for MatMulQhpiHvx8RowRhsPrepack
...
{
  /*
   * Shape legal function for MatMulQhpiHvx8RowRhsPrepack
...
{
  /*
   * Build tile function for MatMulQhpiHvx8RowRhsPrepack
...
{
  /*
   * Late rewrite function for MatMulQhpiHvx8RowRhsPrepack
...
// Array of all MatMulQhpiHvx8RowRhsPrepack operations for registration
extern QHPI_OpInfo_v1 matmulqhpihvxpackrhsOpInfo;

static QHPI_OpInfo_v1 matmulqhpihvx8rowrhsprepack_ops[] = {
    matmulqhpihvx8rowrhsprepackOpInfo,
    matmulqhpihvxpackrhsOpInfo,
};

// Registration function for MatMulQhpiHvx8RowRhsPrepack operations
extern "C" void register_matmulqhpihvx8rowrhsprepack_ops()
{
    qhpi_register_ops_v1(sizeof(matmulqhpihvx8rowrhsprepack_ops) / sizeof(matmulqhpihvx8rowrhsprepack_ops[0]), matmulqhpihvx8rowrhsprepack_ops, THIS_PKG_NAME_STR);
}
