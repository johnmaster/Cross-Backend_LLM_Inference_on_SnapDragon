...
输入和参考输出与 `04_matmul_qhpi_hvx` 使用相同的数据，便于直接比较 builtin
MatMul 与自定义 HVX kernel。

## 目录内容

```text
15_matmul_qnn_builtin/
...
/home/lingbok/Qualcomm/qairt/2.47.0.260601/bin/x86_64-linux-clang/qnn-model-lib-generator \
  -c qnn_custom_ops/15_matmul_qnn_builtin/model/matmul_qnn_builtin_model.cpp \
  -t aarch64-android \
  -l matmul_qnn_builtin_model \
  -o qnn_custom_ops/15_matmul_qnn_builtin/model_libs
```

生成文件：

```text
qnn_custom_ops/15_matmul_qnn_builtin/model_libs/aarch64-android/
...
```bash
adb shell 'mkdir -p /data/local/tmp/qnn/15_matmul_qnn_builtin/{lib,input,output}'

adb push \
  qnn_custom_ops/15_matmul_qnn_builtin/model_libs/aarch64-android/libmatmul_qnn_builtin_model.so \
  /data/local/tmp/qnn/15_matmul_qnn_builtin/lib/

adb push \
  qnn_custom_ops/15_matmul_qnn_builtin/input/lhs.raw \
  qnn_custom_ops/15_matmul_qnn_builtin/input/rhs.raw \
  qnn_custom_ops/15_matmul_qnn_builtin/input/input_list.txt \
  /data/local/tmp/qnn/15_matmul_qnn_builtin/input/
```
...
./bin/qnn-net-run \
  ...
  --model 15_matmul_qnn_builtin/lib/libmatmul_qnn_builtin_model.so \
  --input_list 15_matmul_qnn_builtin/input/input_list.txt \
  --output_dir 15_matmul_qnn_builtin/output \
...
actual=np.fromfile("qnn_custom_ops/15_matmul_qnn_builtin/device_output/Result_0/output.raw", dtype=np.float32); \
expected=np.fromfile("qnn_custom_ops/15_matmul_qnn_builtin/test_data/expected_float.raw", dtype=np.float32); \
