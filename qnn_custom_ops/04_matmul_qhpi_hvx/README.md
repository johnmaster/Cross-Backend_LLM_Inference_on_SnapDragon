...
qnn-op-package-generator \
  --config_path "$REPO/qnn_custom_ops/04_matmul_qhpi_hvx/config/MatMulQhpiHvxOpPackage.xml" \
  --output_path "$REPO/qnn_custom_ops/04_matmul_qhpi_hvx/htp" \
  --debug
```
...
```bash
cd "$REPO/qnn_custom_ops/04_matmul_qhpi_hvx/htp/MatMulQhpiHvxOpPackage"
make htp_v75
make htp_aarch64
```
...
```bash
python3 qnn_custom_ops/04_matmul_qhpi_hvx/scripts/generate_inputs.py
```
