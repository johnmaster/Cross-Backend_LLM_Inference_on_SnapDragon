...
本目录是在 `../05_matmul_qhpi_hvx_multi_row` 基础上的累计优化版本：
...
```text
Directory:
  06_matmul_qhpi_hvx_multi_row_vector_convert
...
```bash
PKG=qnn_custom_ops/06_matmul_qhpi_hvx_multi_row_vector_convert/htp/MatMulQhpiHvxMultiRowVectorConvertOpPackage
```
...
/home/lingbok/Qualcomm/qairt/2.47.0.260601/bin/x86_64-linux-clang/qnn-model-lib-generator \
  -c qnn_custom_ops/06_matmul_qhpi_hvx_multi_row_vector_convert/model/custom_matmul_qhpi_hvx_multi_row_vector_convert_model.cpp \
  -t aarch64-android \
  -l custom_matmul_qhpi_hvx_multi_row_vector_convert_model \
  -o qnn_custom_ops/06_matmul_qhpi_hvx_multi_row_vector_convert/model_libs
```
...
与 `05_matmul_qhpi_hvx_multi_row` 的输出：
