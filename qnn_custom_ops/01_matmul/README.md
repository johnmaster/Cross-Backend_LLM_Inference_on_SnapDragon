...
qnn-op-package-generator \
  --config_path "$REPO/qnn_custom_ops/01_matmul/config/MatMulCustomOpPackageHtp.xml" \
  --output_path "$REPO/qnn_custom_ops/01_matmul/htp" \
  --debug
```
...
```bash
cd "$REPO/qnn_custom_ops/01_matmul/htp/MatMulCustomOpPackage"
...
qnn-model-lib-generator \
  -c "$REPO/qnn_custom_ops/01_matmul/model/custom_matmul_htp_model.cpp" \
  -t aarch64-android \
  -l custom_matmul_htp_model \
  -o "$REPO/qnn_custom_ops/01_matmul/model_libs"
```
...
```bash
python3 "$REPO/qnn_custom_ops/01_matmul/scripts/generate_inputs.py"
```
...
```bash
export MATMUL_ROOT="$REPO/qnn_custom_ops/01_matmul"
...
root = Path.home() / "Cross-Backend_LLM_Inference_on_SnapDragon/qnn_custom_ops/01_matmul"
