//==============================================================================
// Auto Generated Code for MatMulQhpiHvx8RowLhsTileCacheFp32Store - QHPI Implementation
...
#define MATMUL_QHPI_HVX_8ROW_LHS_TILE_CACHE_FP32_STORE_INTRINSICS 1
#endif

// Forward declarations for MatMulQhpiHvx8RowLhsTileCacheFp32Store kernel matmulqhpihvx8rowlhstilecachefp32store_float_16_
static uint32_t matmulqhpihvx8rowlhstilecachefp32store_float_16_Execute(QHPI_RuntimeHandle *handle,
                                      uint32_t num_outputs, QHPI_Tensor **outputs,
                                      uint32_t num_inputs, const QHPI_Tensor *const *inputs);
static float matmulqhpihvx8rowlhstilecachefp32store_float_16_CostFunc(const uint32_t num_inputs, const QHPI_Tensor *const *inputs);

// Common forward declarations for MatMulQhpiHvx8RowLhsTileCacheFp32Store
static const QHPI_Op* matmulqhpihvx8rowlhstilecachefp32storeEarlyRewrite(const QHPI_Op *op);
static QHPI_Shape matmulqhpihvx8rowlhstilecachefp32storeShapeRequired(const QHPI_Op *op);
static QHPI_Shape matmulqhpihvx8rowlhstilecachefp32storeShapeLegal(const QHPI_Op *op, const QHPI_Shape* shape);
static const QHPI_Op* matmulqhpihvx8rowlhstilecachefp32storeBuildTile(const QHPI_Op *op, const QHPI_Shape* start, const QHPI_Shape* extent);
static const QHPI_Op* matmulqhpihvx8rowlhstilecachefp32storeLateRewrite(const QHPI_Op *op);

static inline float matmulqhpihvx8rowlhstilecachefp32storeScalarDot(const int16_t *lhs,
                                           const __fp16 *rhs,
                                           uint64_t lhs_base,
                                           uint64_t rhs_base,
                                           uint32_t k,
...
static inline int16_t matmulqhpihvx8rowlhstilecachefp32storeFloatToQ13(float value) {
...
#if MATMUL_QHPI_HVX_8ROW_LHS_TILE_CACHE_FP32_STORE_INTRINSICS
static inline HVX_Vector
matmulqhpihvx8rowlhstilecachefp32storeConvert64Fp16ToQ13(
    const __fp16 *values) {
...
static inline HVX_Vector
matmulqhpihvx8rowlhstilecachefp32storeConvertQ26ToFp32(HVX_Vector values) {
...
static inline void matmulqhpihvx8rowlhstilecachefp32storePackLhsRowsToQ13(
    const __fp16 *lhs,
    int16_t *lhs_q13,
    uint64_t lhs_base,
    uint32_t rows,
  ...
  for (uint32_t row = 0; row < rows; ++row) {
    ...
    for (; reduction < k; ++reduction) {
      lhs_q13[dst_base + reduction] =
          matmulqhpihvx8rowlhstilecachefp32storeFloatToQ13(
...
#if MATMUL_QHPI_HVX_8ROW_LHS_TILE_CACHE_FP32_STORE_INTRINSICS
static inline void matmulqhpihvx8rowlhstilecachefp32storeCompute1x64Hvx(const int16_t *lhs,
                                                       const __fp16 *rhs,
                                                       const float *bias,
                                                       float *output,
                                                       uint64_t lhs_base,
                                                       ...
                                                       uint32_t n,
                                                       uint32_t col) {
#if defined(MATMUL_QHPI_HVX_8ROW_LHS_TILE_CACHE_FP32_STORE_DEBUG_STORE_RHS)
  (void)lhs;
  (void)lhs_base;
  (void)k;
  ...
  const HVX_Vector even =
      matmulqhpihvx8rowlhstilecachefp32storeConvertQ26ToFp32(Q6_V_lo_W(acc));
  const HVX_Vector odd =
      matmulqhpihvx8rowlhstilecachefp32storeConvertQ26ToFp32(Q6_V_hi_W(acc));
  const HVX_VectorPair interleaved = Q6_W_vshuff_VVR(odd, even, -4);
  HVX_Vector out_lo = Q6_V_lo_W(interleaved);
  HVX_Vector out_hi = Q6_V_hi_W(interleaved);
  if (bias != nullptr) {
    const HVX_Vector bias_lo = vmemu(&bias[col]);
    const HVX_Vector bias_hi = vmemu(&bias[col + 32]);
    out_lo = Q6_Vsf_vadd_VsfVsf(out_lo, bias_lo);
    out_hi = Q6_Vsf_vadd_VsfVsf(out_hi, bias_hi);
  }
  vmemu(&output[output_base + col]) = out_lo;
  vmemu(&output[output_base + col + 32]) = out_hi;
#endif
}

static inline void matmulqhpihvx8rowlhstilecachefp32storeCompute8x64Hvx(
    const int16_t *lhs,
    const __fp16 *rhs,
    const float *bias,
    float *output,
  ...
  for (uint32_t row = 0; row < 8; ++row) {
    ...
    const HVX_Vector even = matmulqhpihvx8rowlhstilecachefp32storeConvertQ26ToFp32(
        Q6_V_lo_W(accumulators[row]));
    const HVX_Vector odd = matmulqhpihvx8rowlhstilecachefp32storeConvertQ26ToFp32(
    ...
    vmemu(&output[row_output_base]) = out_lo;
    vmemu(&output[row_output_base + 32]) = out_hi;
  }
#endif
}
#endif
...
/*
 * QHPI Registration using hexnn_ffi.h API for MatMulQhpiHvx8RowLhsTileCacheFp32Store
 * Multiple kernels for different data type combinations
 */


// Input tensor signatures for MatMulQhpiHvx8RowLhsTileCacheFp32Store kernel matmulqhpihvx8rowlhstilecachefp32store_float_16_
// Includes both regular inputs and parameters as inputs
static QHPI_Tensor_Signature_v1 matmulqhpihvx8rowlhstilecachefp32store_float_16_InputSignatures[] = {
...
static QHPI_Tensor_Signature_v1 matmulqhpihvx8rowlhstilecachefp32store_float_16_OutputSignatures[] = {
...
// Kernel definition for MatMulQhpiHvx8RowLhsTileCacheFp32Store kernel matmulqhpihvx8rowlhstilecachefp32store_float_16_
static QHPI_Kernel_v1 matmulqhpihvx8rowlhstilecachefp32store_float_16_Kernel = {
    .function_name = "matmulqhpihvx8rowlhstilecachefp32store_float_16_Execute",
    .function = matmulqhpihvx8rowlhstilecachefp32store_float_16_Execute,
    ...
    .min_inputs = 2,
    .input_signature = matmulqhpihvx8rowlhstilecachefp32store_float_16_InputSignatures,
    .min_outputs = 1,
    .output_signature = matmulqhpihvx8rowlhstilecachefp32store_float_16_OutputSignatures,
    .cost_function = matmulqhpihvx8rowlhstilecachefp32store_float_16_CostFunc,
    .sync_block_size = 0,
...
// Array of all kernels for MatMulQhpiHvx8RowLhsTileCacheFp32Store
static QHPI_Kernel_v1 matmulqhpihvx8rowlhstilecachefp32storeKernels[] = {

    matmulqhpihvx8rowlhstilecachefp32store_float_16_Kernel
};

// Operator info for MatMulQhpiHvx8RowLhsTileCacheFp32Store - exported for package registration
QHPI_OpInfo_v1 matmulqhpihvx8rowlhstilecachefp32storeOpInfo = {
    .name = THIS_PKG_NAME_STR "::" "MatMulQhpiHvx8RowLhsTileCacheFp32Store",
    .num_kernels = 1,
    .kernels = matmulqhpihvx8rowlhstilecachefp32storeKernels,
    .early_rewrite = matmulqhpihvx8rowlhstilecachefp32storeEarlyRewrite,
    .shape_required = matmulqhpihvx8rowlhstilecachefp32storeShapeRequired,
    .shape_legalized = matmulqhpihvx8rowlhstilecachefp32storeShapeLegal,
    .build_tile = matmulqhpihvx8rowlhstilecachefp32storeBuildTile,
    .late_rewrite = matmulqhpihvx8rowlhstilecachefp32storeLateRewrite
};


/* QHPI execute function implementation for MatMulQhpiHvx8RowLhsTileCacheFp32Store kernel matmulqhpihvx8rowlhstilecachefp32store_float_16_ */
static uint32_t matmulqhpihvx8rowlhstilecachefp32store_float_16_Execute(
  ...
  if ((num_inputs != 2 && num_inputs != 3) || num_outputs != 1) {
  ...
  if (num_inputs == 3 && inputs[2] == nullptr) {
    return QHPI_ErrorFatal;
  }

  const QHPI_Shape lhs_shape = qhpi_tensor_shape(inputs[0]);
  const QHPI_Shape rhs_shape = qhpi_tensor_shape(inputs[1]);
  const QHPI_Shape bias_shape =
      num_inputs == 3 ? qhpi_tensor_shape(inputs[2]) : QHPI_Shape{0};
  ...
  if (num_inputs == 3) {
    const bool bias_rank1 = bias_shape.rank == 1 && bias_shape.dims[0] == n;
    const bool bias_rank4 = bias_shape.rank == 4 &&
                            bias_shape.dims[0] == 1 &&
                            bias_shape.dims[1] == 1 &&
                            bias_shape.dims[2] == 1 &&
                            bias_shape.dims[3] == n;
    if (!bias_rank1 && !bias_rank4) {
      return QHPI_ErrorFatal;
    }
  }
  ...
  // LHS is converted to Q13 once per row tile instead of once per output
  // column block.
  for (uint32_t b = 0; b < batch; ++b) {
    for (uint32_t h = 0; h < heads; ++h) {
      ...
      uint32_t row = 0;
#if MATMUL_QHPI_HVX_8ROW_LHS_TILE_CACHE_FP32_STORE_INTRINSICS && \
    !defined(MATMUL_QHPI_HVX_8ROW_LHS_TILE_CACHE_FP32_STORE_FORCE_SCALAR)
      for (; row + 8 <= m; row += 8) {
        ...
        matmulqhpihvx8rowlhstilecachefp32storePackLhsRowsToQ13(
            lhs, lhs_tile_q13, lhs_base, 8, k);

        uint32_t col = 0;
        for (; col + 64 <= n; col += 64) {
          matmulqhpihvx8rowlhstilecachefp32storeCompute8x64Hvx(
              lhs_tile_q13,
              rhs,
              bias,
              output,
        ...
        for (; col < n; ++col) {
          for (uint32_t tile_row = 0; tile_row < 8; ++tile_row) {
            ...
            output[tile_output_base + col] =
                matmulqhpihvx8rowlhstilecachefp32storeScalarDot(
                    lhs_tile_q13, rhs, tile_lhs_base, rhs_base, k, n, col) +
                (bias != nullptr ? bias[col] : 0.0f);
          }
        }
      }
#endif
      ...
      for (; row < m; ++row) {
        ...
        const uint64_t output_base =
            (((uint64_t)b * heads + h) * m + row) * n;
        alignas(128) int16_t lhs_tile_q13[kMaxTileCacheReduction];
        matmulqhpihvx8rowlhstilecachefp32storePackLhsRowsToQ13(
            lhs, lhs_tile_q13, lhs_base, 1, k);

        uint32_t col = 0;
#if MATMUL_QHPI_HVX_8ROW_LHS_TILE_CACHE_FP32_STORE_INTRINSICS && \
    !defined(MATMUL_QHPI_HVX_8ROW_LHS_TILE_CACHE_FP32_STORE_FORCE_SCALAR)
        for (; col + 64 <= n; col += 64) {
          matmulqhpihvx8rowlhstilecachefp32storeCompute1x64Hvx(
              lhs_tile_q13,
              rhs,
              bias,
              output,
        ...
        for (; col < n; ++col) {
          output[output_base + col] =
              matmulqhpihvx8rowlhstilecachefp32storeScalarDot(
                  lhs_tile_q13, rhs, 0, rhs_base, k, n, col) +
              (bias != nullptr ? bias[col] : 0.0f);
        }
      }
    }
...
{
  /*
   * Cost estimation function for MatMulQhpiHvx8RowLhsTileCacheFp32Store kernel matmulqhpihvx8rowlhstilecachefp32store_float_16_
...
/*
 * Common stub implementations for MatMulQhpiHvx8RowLhsTileCacheFp32Store QHPI_OpInfo functions
...
{
  /*
   * Early rewrite function for MatMulQhpiHvx8RowLhsTileCacheFp32Store
...
{
  /*
   * Shape required function for MatMulQhpiHvx8RowLhsTileCacheFp32Store
...
{
  /*
   * Shape legal function for MatMulQhpiHvx8RowLhsTileCacheFp32Store
...
{
  /*
   * Build tile function for MatMulQhpiHvx8RowLhsTileCacheFp32Store
...
{
  /*
   * Late rewrite function for MatMulQhpiHvx8RowLhsTileCacheFp32Store
...
// Array of all MatMulQhpiHvx8RowLhsTileCacheFp32Store operations for registration
static QHPI_OpInfo_v1 matmulqhpihvx8rowlhstilecachefp32store_ops[] = {
    matmulqhpihvx8rowlhstilecachefp32storeOpInfo
};

// Registration function for MatMulQhpiHvx8RowLhsTileCacheFp32Store operations
extern "C" void register_matmulqhpihvx8rowlhstilecachefp32store_ops()
{
    qhpi_register_ops_v1(sizeof(matmulqhpihvx8rowlhstilecachefp32store_ops) / sizeof(matmulqhpihvx8rowlhstilecachefp32store_ops[0]), matmulqhpihvx8rowlhstilecachefp32store_ops, THIS_PKG_NAME_STR);
}
