//==============================================================================
// Auto Generated Code for MatMulQhpiHvx8RowFp32Store - QHPI Implementation
...
#define MATMUL_QHPI_HVX_8ROW_FP32_STORE_INTRINSICS 1
#endif

// Forward declarations for MatMulQhpiHvx8RowFp32Store kernel matmulqhpihvx8rowfp32store_float_16_
static uint32_t matmulqhpihvx8rowfp32store_float_16_Execute(QHPI_RuntimeHandle *handle,
                                      uint32_t num_outputs, QHPI_Tensor **outputs,
                                      uint32_t num_inputs, const QHPI_Tensor *const *inputs);
static float matmulqhpihvx8rowfp32store_float_16_CostFunc(const uint32_t num_inputs, const QHPI_Tensor *const *inputs);

// Common forward declarations for MatMulQhpiHvx8RowFp32Store
static const QHPI_Op* matmulqhpihvx8rowfp32storeEarlyRewrite(const QHPI_Op *op);
static QHPI_Shape matmulqhpihvx8rowfp32storeShapeRequired(const QHPI_Op *op);
static QHPI_Shape matmulqhpihvx8rowfp32storeShapeLegal(const QHPI_Op *op, const QHPI_Shape* shape);
static const QHPI_Op* matmulqhpihvx8rowfp32storeBuildTile(const QHPI_Op *op, const QHPI_Shape* start, const QHPI_Shape* extent);
static const QHPI_Op* matmulqhpihvx8rowfp32storeLateRewrite(const QHPI_Op *op);

static inline float matmulqhpihvx8rowfp32storeScalarDot(const __fp16 *lhs,
                                           const __fp16 *rhs,
                                           uint64_t lhs_base,
                                           uint64_t rhs_base,
                                           uint32_t k,
...
static inline int16_t matmulqhpihvx8rowfp32storeFloatToQ13(float value) {
...
#if MATMUL_QHPI_HVX_8ROW_FP32_STORE_INTRINSICS
static inline HVX_Vector
matmulqhpihvx8rowfp32storeConvert64Fp16ToQ13(
    const __fp16 *values) {
...
static inline void matmulqhpihvx8rowfp32storeCompute1x64Hvx(const __fp16 *lhs,
                                                       const __fp16 *rhs,
                                                       float *output,
                                                       uint64_t lhs_base,
                                                       uint64_t rhs_base,
                                                       ...
                                                       uint32_t n,
                                                       uint32_t col) {
#if defined(MATMUL_QHPI_HVX_8ROW_FP32_STORE_DEBUG_STORE_RHS)
  (void)lhs;
  (void)lhs_base;
  (void)k;
  ...
  for (uint32_t reduction = 0; reduction < k; ++reduction) {
    const int16_t lhs_q13 =
        matmulqhpihvx8rowfp32storeFloatToQ13(static_cast<float>(lhs[lhs_base + reduction]));
  ...
  const HVX_Vector even =
      matmulqhpihvx8rowfp32storeConvertQ26ToFp32(Q6_V_lo_W(acc));
  const HVX_Vector odd =
      matmulqhpihvx8rowfp32storeConvertQ26ToFp32(Q6_V_hi_W(acc));
...
static inline void matmulqhpihvx8rowfp32storeCompute8x64Hvx(
    const __fp16 *lhs,
    const __fp16 *rhs,
    float *output,
    uint64_t lhs_base,
  ...
  for (uint32_t reduction = 0; reduction < k; ++reduction) {
    ...
    const HVX_Vector lhs0 = Q6_Vh_vsplat_R(
        matmulqhpihvx8rowfp32storeFloatToQ13(
  ...
  for (uint32_t row = 0; row < 8; ++row) {
    ...
    const HVX_Vector even = matmulqhpihvx8rowfp32storeConvertQ26ToFp32(
        Q6_V_lo_W(accumulators[row]));
    const HVX_Vector odd = matmulqhpihvx8rowfp32storeConvertQ26ToFp32(
...
/*
 * QHPI Registration using hexnn_ffi.h API for MatMulQhpiHvx8RowFp32Store
 * Multiple kernels for different data type combinations
 */


// Input tensor signatures for MatMulQhpiHvx8RowFp32Store kernel matmulqhpihvx8rowfp32store_float_16_
// Includes both regular inputs and parameters as inputs
static QHPI_Tensor_Signature_v1 matmulqhpihvx8rowfp32store_float_16_InputSignatures[] = {
...
static QHPI_Tensor_Signature_v1 matmulqhpihvx8rowfp32store_float_16_OutputSignatures[] = {
...
// Kernel definition for MatMulQhpiHvx8RowFp32Store kernel matmulqhpihvx8rowfp32store_float_16_
static QHPI_Kernel_v1 matmulqhpihvx8rowfp32store_float_16_Kernel = {
    .function_name = "matmulqhpihvx8rowfp32store_float_16_Execute",
    .function = matmulqhpihvx8rowfp32store_float_16_Execute,
    ...
    .min_inputs = 2,
    .input_signature = matmulqhpihvx8rowfp32store_float_16_InputSignatures,
    .min_outputs = 1,
    .output_signature = matmulqhpihvx8rowfp32store_float_16_OutputSignatures,
    .cost_function = matmulqhpihvx8rowfp32store_float_16_CostFunc,
    .sync_block_size = 0,
...
// Array of all kernels for MatMulQhpiHvx8RowFp32Store
static QHPI_Kernel_v1 matmulqhpihvx8rowfp32storeKernels[] = {

    matmulqhpihvx8rowfp32store_float_16_Kernel
};

// Operator info for MatMulQhpiHvx8RowFp32Store - exported for package registration
QHPI_OpInfo_v1 matmulqhpihvx8rowfp32storeOpInfo = {
    .name = THIS_PKG_NAME_STR "::" "MatMulQhpiHvx8RowFp32Store",
    .num_kernels = 1,
    .kernels = matmulqhpihvx8rowfp32storeKernels,
    .early_rewrite = matmulqhpihvx8rowfp32storeEarlyRewrite,
    .shape_required = matmulqhpihvx8rowfp32storeShapeRequired,
    .shape_legalized = matmulqhpihvx8rowfp32storeShapeLegal,
    .build_tile = matmulqhpihvx8rowfp32storeBuildTile,
    .late_rewrite = matmulqhpihvx8rowfp32storeLateRewrite
};


/* QHPI execute function implementation for MatMulQhpiHvx8RowFp32Store kernel matmulqhpihvx8rowfp32store_float_16_ */
static uint32_t matmulqhpihvx8rowfp32store_float_16_Execute(
  ...
  for (uint32_t b = 0; b < batch; ++b) {
    for (uint32_t h = 0; h < heads; ++h) {
      ...
      uint32_t row = 0;
#if MATMUL_QHPI_HVX_8ROW_FP32_STORE_INTRINSICS && \
    !defined(MATMUL_QHPI_HVX_8ROW_FP32_STORE_FORCE_SCALAR)
      for (; row + 8 <= m; row += 8) {
        ...
        uint32_t col = 0;
        for (; col + 64 <= n; col += 64) {
          matmulqhpihvx8rowfp32storeCompute8x64Hvx(
        ...
        for (; col < n; ++col) {
          for (uint32_t tile_row = 0; tile_row < 8; ++tile_row) {
            ...
            output[tile_output_base + col] =
                matmulqhpihvx8rowfp32storeScalarDot(
    ...
    !defined(MATMUL_QHPI_HVX_8ROW_FP32_STORE_FORCE_SCALAR)
        for (; col + 64 <= n; col += 64) {
          matmulqhpihvx8rowfp32storeCompute1x64Hvx(
              lhs, rhs, output, lhs_base, rhs_base, output_base, k, n, col);
        }
#endif
        for (; col < n; ++col) {
...
{
  /*
   * Cost estimation function for MatMulQhpiHvx8RowFp32Store kernel matmulqhpihvx8rowfp32store_float_16_
...
/*
 * Common stub implementations for MatMulQhpiHvx8RowFp32Store QHPI_OpInfo functions
...
{
  /*
   * Early rewrite function for MatMulQhpiHvx8RowFp32Store
...
{
  /*
   * Shape required function for MatMulQhpiHvx8RowFp32Store
...
{
  /*
   * Shape legal function for MatMulQhpiHvx8RowFp32Store
...
{
  /*
   * Build tile function for MatMulQhpiHvx8RowFp32Store
...
{
  /*
   * Late rewrite function for MatMulQhpiHvx8RowFp32Store
...
// Array of all MatMulQhpiHvx8RowFp32Store operations for registration
static QHPI_OpInfo_v1 matmulqhpihvx8rowfp32store_ops[] = {
    matmulqhpihvx8rowfp32storeOpInfo
};

// Registration function for MatMulQhpiHvx8RowFp32Store operations
extern "C" void register_matmulqhpihvx8rowfp32store_ops()
{
    qhpi_register_ops_v1(sizeof(matmulqhpihvx8rowfp32store_ops) / sizeof(matmulqhpihvx8rowfp32store_ops[0]), matmulqhpihvx8rowfp32store_ops, THIS_PKG_NAME_STR);
}
