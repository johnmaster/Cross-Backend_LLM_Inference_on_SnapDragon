...
对应的 CPU 版本见 [../cpu/README.md](../cpu/README.md)。HTP 与 CPU 最大的区别是：HTP 在线准备流程需要同时提供 ARM64 和 Hexagon 两个架构的 Op Package 动态库。
...
Lab root        : docs/QNN/custom_op_packages/relu/htp
```
...
model_libs/aarch64-android/libcustom_relu_htp_model.so
...
qnn-op-package-generator \
  ...
  --output_path docs/QNN/custom_op_packages/relu/htp \
  --debug
```
...
```text
docs/QNN/custom_op_packages/relu/htp/ReluOpPackage/
├── Makefile
...
cd docs/QNN/custom_op_packages/relu/htp/ReluOpPackage
```
...
CPU 测试模型使用二维 `[1,4]`。HTP `reluImpl()` 按 BHWD 四维接口访问，因此 HTP 模型改为 `[1,1,1,4]`：

```bash
docs/QNN/custom_op_packages/relu/htp/model/custom_relu_htp_model.cpp
```
...
qnn-model-lib-generator \
  -c docs/QNN/custom_op_packages/relu/htp/model/custom_relu_htp_model.cpp \
  -t aarch64-android \
  -l custom_relu_htp_model \
  -o docs/QNN/custom_op_packages/relu/htp/model_libs
```

产物：

```text
docs/QNN/custom_op_packages/relu/htp/model_libs/aarch64-android/libcustom_relu_htp_model.so
```
...
```bash
mkdir -p docs/QNN/custom_op_packages/relu/htp/test_data

python3 - <<'PY'
import numpy as np

x = np.array([-2.0, -0.5, 1.5, 3.0], dtype=np.float32)
x.tofile("docs/QNN/custom_op_packages/relu/htp/test_data/input.raw")
print("input:", x)
PY
```
...
```bash
adb push \
  docs/QNN/custom_op_packages/relu/htp/ReluOpPackage/build/aarch64-android/libQnnReluOpPackage.so \
...
```bash
adb push \
  docs/QNN/custom_op_packages/relu/htp/ReluOpPackage/build/hexagon-v75/libQnnReluOpPackage.so \
...
```bash
adb push \
  docs/QNN/custom_op_packages/relu/htp/test_data/input.raw \
...
```bash
rm -rf docs/QNN/custom_op_packages/relu/htp/output

adb pull \
  /data/local/tmp/qnn/custom_relu_htp/output \
  docs/QNN/custom_op_packages/relu/htp/output
```
...
root = Path("docs/QNN/custom_op_packages/relu/htp")

input_data = np.fromfile(root / "test_data/input.raw", dtype=np.float32)
output_file = next((root / "output").rglob("*.raw"))
