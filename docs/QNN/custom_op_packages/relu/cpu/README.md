...
qnn-op-package-generator \
  ...
  --output_path docs/QNN/custom_op_packages/relu/cpu \
  --debug
```
...
```text
docs/QNN/custom_op_packages/relu/cpu/ReluOpPackage/
├── Makefile
├── config/
...
```bash
cd docs/QNN/custom_op_packages/relu/cpu/ReluOpPackage
...
```text
docs/QNN/custom_op_packages/relu/cpu/model/custom_relu_model.cpp
```
...
qnn-model-lib-generator \
  -c docs/QNN/custom_op_packages/relu/cpu/model/custom_relu_model.cpp \
  -t aarch64-android \
  -l custom_relu_model \
  -o docs/QNN/custom_op_packages/relu/cpu/model_libs
```

产物：

```text
docs/QNN/custom_op_packages/relu/cpu/model_libs/aarch64-android/libcustom_relu_model.so
```
...
```bash
mkdir -p docs/QNN/custom_op_packages/relu/cpu/test_data

python3 - <<'PY'
import struct

values = [-2.0, -0.5, 1.0, 3.0]
with open("docs/QNN/custom_op_packages/relu/cpu/test_data/input.raw", "wb") as f:
...
printf '%s\n' \
  'input:=/data/local/tmp/qnn/custom_relu/input/input.raw' \
  > docs/QNN/custom_op_packages/relu/cpu/test_data/input_list.txt
```
...
adb push \
  docs/QNN/custom_op_packages/relu/cpu/ReluOpPackage/libs/aarch64-android/libReluOpPackage.so \
  /data/local/tmp/qnn/custom_relu/lib/

adb push \
  docs/QNN/custom_op_packages/relu/cpu/ReluOpPackage/libs/aarch64-android/libc++_shared.so \
  /data/local/tmp/qnn/custom_relu/lib/
```
...
```bash
adb push docs/QNN/custom_op_packages/relu/cpu/test_data/input.raw \
  /data/local/tmp/qnn/custom_relu/input/

adb push docs/QNN/custom_op_packages/relu/cpu/test_data/input_list.txt \
  /data/local/tmp/qnn/custom_relu/input/
```
...
adb pull \
  ...
  docs/QNN/custom_op_packages/relu/cpu/output.raw
```
...
path = "docs/QNN/custom_op_packages/relu/cpu/output.raw"
