                           ...
                           (Qnn_Tensor_t) {
                               ...
                               {.v2= {
                                 ...
                                 .quantizeParams= { QNN_DEFINITION_DEFINED,
                                                    ...
                                                    {.scaleOffsetEncoding= {.scale= 0.0000152582797454670071601867675781250000f, .offset= -32768}}},
                                 .rank= 4,
  ...
  Qnn_Tensor_t outputs_MatMul[] = {
    (Qnn_Tensor_t) {
          ...
          {.v2= {
            ...
            .quantizeParams= { QNN_DEFINITION_DEFINED,
                               QNN_QUANTIZATION_ENCODING_SCALE_OFFSET,
                               {.scaleOffsetEncoding= {.scale= 0.0001227317261509597301483154296875000000f, .offset= -31933}}},
            .rank= 4,
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for w8a16_matmul*/
  QnnModel w8a16_matmul;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("w8a16_matmul", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(w8a16_matmul.initialize(backendHandle, interface, contextHandle, "w8a16_matmul", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_lhs(w8a16_matmul), err);
  VALIDATE(addNode_lhs_nchw(w8a16_matmul), err);
  VALIDATE(addTensor_rhs_nchw(w8a16_matmul), err);
  VALIDATE(addNode_MatMul(w8a16_matmul), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&w8a16_matmul};
