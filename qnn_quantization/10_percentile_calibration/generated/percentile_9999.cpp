  ...
  Qnn_Tensor_t outputs_MatMul[] = {
    (Qnn_Tensor_t) {
          ...
          {.v2= {
            ...
            .quantizeParams= { QNN_DEFINITION_DEFINED,
                               QNN_QUANTIZATION_ENCODING_SCALE_OFFSET,
                               {.scaleOffsetEncoding= {.scale= 0.0281743649393320083618164062500000000000f, .offset= -116}}},
            .rank= 4,
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for percentile_9999*/
  QnnModel percentile_9999;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("percentile_9999", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(percentile_9999.initialize(backendHandle, interface, contextHandle, "percentile_9999", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_lhs(percentile_9999), err);
  VALIDATE(addNode_lhs_nchw(percentile_9999), err);
  VALIDATE(addTensor_rhs_nchw(percentile_9999), err);
  VALIDATE(addNode_MatMul(percentile_9999), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&percentile_9999};
