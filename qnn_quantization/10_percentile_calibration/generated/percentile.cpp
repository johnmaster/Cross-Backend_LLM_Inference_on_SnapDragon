                           ...
                           (Qnn_Tensor_t) {
                               ...
                               {.v2= {
                                 ...
                                 .quantizeParams= { QNN_DEFINITION_DEFINED,
                                                    ...
                                                    {.scaleOffsetEncoding= {.scale= 0.0039062425494194030761718750000000000000f, .offset= -124}}},
                                 .rank= 4,
  ...
  Qnn_Tensor_t outputs_MatMul[] = {
    (Qnn_Tensor_t) {
          ...
          {.v2= {
            ...
            .quantizeParams= { QNN_DEFINITION_DEFINED,
                               QNN_QUANTIZATION_ENCODING_SCALE_OFFSET,
                               {.scaleOffsetEncoding= {.scale= 0.0216590389609336853027343750000000000000f, .offset= -123}}},
            .rank= 4,
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for percentile*/
  QnnModel percentile;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("percentile", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(percentile.initialize(backendHandle, interface, contextHandle, "percentile", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_lhs(percentile), err);
  VALIDATE(addNode_lhs_nchw(percentile), err);
  VALIDATE(addTensor_rhs_nchw(percentile), err);
  VALIDATE(addNode_MatMul(percentile), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&percentile};
