...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for int8_per_axis_qdq_matmul*/
  QnnModel int8_per_axis_qdq_matmul;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("int8_per_axis_qdq_matmul", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(int8_per_axis_qdq_matmul.initialize(backendHandle, interface, contextHandle, "int8_per_axis_qdq_matmul", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_lhs(int8_per_axis_qdq_matmul), err);
  VALIDATE(addNode_lhs_nchw(int8_per_axis_qdq_matmul), err);
  VALIDATE(addTensor_rhs_dequantized_nchw(int8_per_axis_qdq_matmul), err);
  VALIDATE(addNode_PerAxisWeightMatMul(int8_per_axis_qdq_matmul), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&int8_per_axis_qdq_matmul};
