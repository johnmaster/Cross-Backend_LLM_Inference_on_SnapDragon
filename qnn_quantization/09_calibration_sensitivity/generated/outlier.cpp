                           ...
                           (Qnn_Tensor_t) {
                               ...
                               {.v2= {
                                 ...
                                 .quantizeParams= { QNN_DEFINITION_DEFINED,
                                                    ...
                                                    {.scaleOffsetEncoding= {.scale= 0.0333332493901252746582031250000000000000f, .offset= -15}}},
                                 .rank= 4,
  ...
  Qnn_Tensor_t outputs_MatMul[] = {
    (Qnn_Tensor_t) {
          ...
          {.v2= {
            ...
            .quantizeParams= { QNN_DEFINITION_DEFINED,
                               QNN_QUANTIZATION_ENCODING_SCALE_OFFSET,
                               {.scaleOffsetEncoding= {.scale= 0.0300526507198810577392578125000000000000f, .offset= -118}}},
            .rank= 4,
...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for outlier*/
  QnnModel outlier;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("outlier", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(outlier.initialize(backendHandle, interface, contextHandle, "outlier", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_lhs(outlier), err);
  VALIDATE(addNode_lhs_nchw(outlier), err);
  VALIDATE(addTensor_rhs_nchw(outlier), err);
  VALIDATE(addNode_MatMul(outlier), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&outlier};
