...
ModelError_t QnnModel_composeGraphs(Qnn_BackendHandle_t backendHandle,
  ...
  /* model/graph for block_32_v2*/
  QnnModel block_32_v2;
  const QnnGraph_Config_t** graphConfigs = nullptr;
  VALIDATE(getQnnGraphConfigFromInfo("block_32_v2", graphsConfigInfo, numGraphsConfigInfo, graphConfigs), err);
  VALIDATE(block_32_v2.initialize(backendHandle, interface, contextHandle, "block_32_v2", debug, DO_GRAPH_NODE_VALIDATIONS, graphConfigs), err);
  VALIDATE(addTensor_lhs(block_32_v2), err);
  VALIDATE(addNode_lhs_nchw(block_32_v2), err);
  VALIDATE(addTensor_rhs_nchw(block_32_v2), err);
  VALIDATE(addNode_MatMul(block_32_v2), err);

  // Add all models to array to get graphsInfo
  QnnModel* models [] = {&block_32_v2};
